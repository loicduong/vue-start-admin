...
const tableRowClassName = ({ row, rowIndex }: RowContext): string => {
  if (rowIndex === 1) {
    return 'warning-row';
  } else if (rowIndex === 3) {
    return 'success-row';
...
const deleteRow = (index: number, rows: TableItem[]) => {
  rows.splice(index, 1);
}
...
const setCurrent = (row: TableItem) => {
...
const formatter = (row: TableItem, column: any) => {
  return row.address;
}
</script>`
...
const filterTable = ref<any>(null)
...
const filterHandler = (value: any, row: TableItem, column: any) => {
  const property = column['property'];
  return row[property as keyof TableItem] === value;
}
</script>`
...
const load = (tree: TableItem, treeNode: any, resolve: (data: TableItem[]) => void) => {
  setTimeout(() => {
    resolve([
      {
        id: 31,
...
const getSummaries = (param: { columns: any[], data: TableItem[] }) => {
  ...
  columns.forEach((column, index) => {
    ...
    const values = data.map(item => Number(item[column.property as keyof TableItem]));

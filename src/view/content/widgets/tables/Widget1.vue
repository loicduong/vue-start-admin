...
const dataToShow = computed(() => {
  if (show.value === day.value)
    return day.value
  if (show.value === week.value)
    return week.value
  if (show.value === month.value)
    return month.value
  return day.value
})
...
<template>
  <div class="card card-custom card-stretch mb-5 mb-xxl-8">
    ...
    <div class="card-header border-0 pt-5">
      ...
      <div class="card-toolbar">
        <ul class="nav nav-pills nav-pills-sm nav-light">
          <li class="nav-item" @click="setCurrentData(day)">
            <a
              class="nav-link btn btn-active-light btn-color-muted py-2 px-4 fw-bolder me-2"
              :class="{ active: show === day }"
            >Day</a>
          </li>
          <li class="nav-item" @click="setCurrentData(week)">
            <a
              class="nav-link btn btn-active-light btn-color-muted py-2 px-4 fw-bolder me-2"
              :class="{ active: show === week }"
            >Week</a>
          </li>
          <li class="nav-item" @click="setCurrentData(month)">
            <a
              class="nav-link btn btn-active-light btn-color-muted py-2 px-4 fw-bolder"
    ...
    <div class="card-body pt-2 pb-0 mt-n3">
      <div id="myTabTables1" class="tab-content mt-5">
        <!-- begin::Tap pane -->
        <div id="kt_tab_pane_1_1" class="tab-pane fade show active" role="tabpanel" aria-labelledby="kt_tab_pane_1_1">
          <!-- begin::Table -->
                  ...
                  <tr>
                    <th class="px-0 py-3">
                      <div class="symbol symbol-65px me-5">
                        <span class="symbol-label" :class="`bg-light-${item.status}`">
                          <span class="svg-icon svg-icon-1" :class="`svg-icon-${item.status}`">
                    ...
                    <td class="ps-0">
                      ...
                      <span class="text-muted fw-bold d-block mt-1 fs-6">
                        {{ item.desc }}
                      </span>
                    </td>
                    <td>
                      <div class="d-flex flex-column w-100 mr-3">
                        <div class="d-flex align-items-center justify-content-between mb-2">
                          <span class="text-dark mr-2 fs-6 fw-bolder">
                            Progress
                          </span>
                        </div>
                        <div class="d-flex align-items-center">
                          <div class="progress h-6px w-100" :class="`bg-light-${item.status}`">
                            <div
                              class="progress-bar"

...
class ScrollComponent {
  ...
  private getAutoHeight = () => {
    ...
    if (dependencies !== null) {
      ...
      if (elements && elements.length > 0) {
        for (let i = 0, len = elements.length; i < len; i++) {
          ...
          height = height - Number.parseInt(getCSS(element, 'height'))
          height = height - Number.parseInt(getCSS(element, 'margin-top'))
          height = height - Number.parseInt(getCSS(element, 'margin-bottom'))

          const borderTop = getCSS(element, 'border-top')
          if (borderTop) {
            height = height - Number.parseInt(borderTop)
          }

          const borderBottom = getCSS(element, 'border-bottom')
          if (borderBottom) {
            height = height - Number.parseInt(borderBottom)
          }
        }
      }
    }
    ...
    if (wrappers !== null) {
      ...
      if (elements && elements.length > 0) {
        for (let i = 0, len = elements.length; i < len; i++) {
          ...
          height = height - Number.parseInt(getCSS(element, 'margin-top'))
          height = height - Number.parseInt(getCSS(element, 'margin-bottom'))
          height = height - Number.parseInt(getCSS(element, 'padding-top'))
          height = height - Number.parseInt(getCSS(element, 'padding-bottom'))
    ...
    // Custom offset
    if (offset !== null) {
      height = height - Number.parseInt(offset as string)
    }

    height = height - Number.parseInt(getCSS(this.element, 'margin-top'))
    height = height - Number.parseInt(getCSS(this.element, 'margin-bottom'))
  ...
  private setupState = () => {
    if (this.getOption('save-state') === true && this.id) {
      ...
      if (cookie) {
        const pos = Number.parseInt(cookie)

...
import { slideDown, slideUp } from '../_utils/helpers/dom-helpers/slide'
...
class MenuComponent {
  ...
  private _getItemSubType = (element: HTMLElement) => {
    ...
    if (sub && Number.parseInt(this._getCss(sub, 'z-index')) > 0) {
      return 'dropdown'
    }
    else {
      return 'accordion'
  ...
  private _showDropdown = (item: HTMLElement) => {
    ...
    computePosition(item, sub, this._getDropdownPopperConfig(item) as Partial<ComputePositionConfig>).then((popper) => {
